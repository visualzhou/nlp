...
public class MaximumEntropyClassifier<I, F, L> implements
	...
	public static class ObjectiveFunction<F, L> implements
		...
		private Pair<Double, double[]> calculate(double[] x) {
			...
			for (int dataIndex = 0; dataIndex < data.length; dataIndex++) {
				EncodedDatum datum = data[dataIndex];

				// 1.1 objective add the score to L(w)
				objective += getScore(datum, x, indexLinearizer,
						datum.labelIndex);

				// 1.2 objective add log(sum(exp^score(xi, y')) for each y' in Y)
				double[] logProbabilities = initLogProbabilities();
				double logSumExpScore = getLogProbabilities(datum, x, encoding,
						indexLinearizer, logProbabilities);
				objective -= logSumExpScore;
				
				for (int featureIndexIndex = 0; featureIndexIndex < datum.getNumActiveFeatures(); featureIndexIndex++) {
					// 2.1 count f(xi) for its real label
					int featureIndex = datum.getFeatureIndex(featureIndexIndex);
					int weightIndex = indexLinearizer.getLinearIndex(featureIndex, datum.getLabelIndex());
					double count = datum.getFeatureCount(featureIndexIndex);
					derivatives[weightIndex] += count;
					
					// 2.2 go through all label to dispatch estimated feature count
					for (int labelIndex = 0; labelIndex < encoding.getNumLabels(); labelIndex++) {
						weightIndex = indexLinearizer.getLinearIndex(featureIndex, labelIndex);
						derivatives[weightIndex] -= SloppyMath.exp(logProbabilities[labelIndex]) * count;
					}
				}
		...
		private double[] initLogProbabilities() {
			return DoubleArrays.constantArray(Double.NEGATIVE_INFINITY,
					encoding.getNumLabels());
		}
	...
	/**
	 * get the probabilities distribution and the normalization factor
	 */
	private static <F, L> double getLogProbabilities(EncodedDatum datum,
			double[] weights, Encoding<F, L> encoding,
			IndexLinearizer indexLinearizer, double[] logProbabilities) {
		// TODO: apply the classifier to this feature vector

		for (int labelIndex = 0; labelIndex < logProbabilities.length; labelIndex++) {

			logProbabilities[labelIndex] = getScore(datum, weights,
					indexLinearizer, labelIndex);
		}
		double denominator = SloppyMath.logAdd(logProbabilities);
		// normalize
		DoubleArrays.addon(logProbabilities, -denominator);
		return denominator;
	}

	private static double getScore(EncodedDatum datum, double[] weights,
			IndexLinearizer indexLinearizer, int labelIndex) {
		double sum = 0.0;
		for (int featureIndexIndex = 0; featureIndexIndex < datum
				.getNumActiveFeatures(); featureIndexIndex++) {
			int wIndex = indexLinearizer.getLinearIndex(
					datum.getFeatureIndex(featureIndexIndex), labelIndex);
			sum += weights[wIndex] * datum.getFeatureCount(featureIndexIndex);
		}
		return sum;
	}
