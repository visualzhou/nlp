...
public class Counters {
	...
	static final double threhold = 1E-30;

	public static <T> Counter<T> cleanCounter(Counter<T> counter) {

		return cleanCounter(counter, threhold);
	}

	public static <T> Counter<T> cleanCounter(Counter<T> counter,
			double threhold) {
		Counter<T> newCounter = new Counter<T>();
		for (T key : counter.keySet()) {
			if (counter.getCount(key) > threhold) {
				newCounter.setCount(key, counter.getCount(key));
			}
		}
		return newCounter;
	}

	public static <K, V> CounterMap<K, V> cleanCounter(
			CounterMap<K, V> countermap) {
		return cleanCounter(countermap, threhold);
	}

	public static <K, V> CounterMap<K, V> cleanCounter(
			CounterMap<K, V> countermap, double threhold) {
		CounterMap<K, V> newCounterMap = new CounterMap<K, V>();
		for (K key : countermap.keySet()) {
			Counter<V> vCounter = countermap.getCounter(key);
			for (V value : vCounter.keySet())
				if (vCounter.getCount(value) > threhold) {
					newCounterMap.incrementCount(key, value,
							vCounter.getCount(value));
				}
		}
